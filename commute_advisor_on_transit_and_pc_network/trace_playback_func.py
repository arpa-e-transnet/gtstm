...
def getTrace(driveFile, network):
    ...
    for i in range(0,len(rt_list)):
        try:
            ...
            print("in trace_playback_func, getTrace")
            pass
    ...
    rt_list['avgspeed'] = rt_list['speed_fts'] * 0.3048
...
def trace2LatLon(rt_pt, startTime, driveFile):
    print("in trace2LatLon, starttime is:", startTime)
    ...
    for i, row in rt_pt.iterrows():
        ...
        #rt_pt.loc[i,'avgspeed'] = row['avgspeed']
    cols = ['date','time','latitude','longitude','secondID','avgspeed']
    finalDf = rt_pt[cols]
...
if __name__=="__main__":
    network = gpd.read_file('data_node_link/abm15_links.shp')
    network.to_csv('network_file_trace.csv', index=False)
    traceFile = getTrace('debug_files/drive_flask_1.csv', network)
